<style>
    ...
    table {
        margin-top: 20px; /* Thêm khoảng cách giữa form và bảng */
        border-collapse: collapse; /* Loại bỏ khoảng trắng giữa các ô trong bảng */
        width: 100%; /* Chiều rộng bảng là 100% */
    }

    th, td {
        border: 1px solid #ddd; /* Đặt đường biên cho các ô trong bảng */
        padding: 8px; /* Thêm khoảng trắng xung quanh nội dung của ô */
        text-align: left; /* Canh trái nội dung trong ô */
    }
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST["search"])) {
        if (!empty($_POST["ipAddress"])) {
            ...
            if (checkAdmin($conn, $_SESSION['username'])) {
            $sqlSearch = "SELECT id, username, status, activated FROM users WHERE ip_address LIKE '%$ipAddress%'";
        ...
        if (!empty($_POST["foundIP"])) {
            ...
            $sqlstatus = "UPDATE users SET status = 1 WHERE ip_address LIKE '%$ipAddress%'";
