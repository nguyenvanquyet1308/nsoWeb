...
<div class="card">
    ...
    <?php include('success.php'); ?>
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            if (checkAdmin($conn, $_SESSION['username'])) {
            if (isset($_POST["edit"])) {
                $news_postsId = $_POST["news_postsId"];

                // Lấy dữ liệu hiện tại từ database
                $sqlSelectCurrentData = "SELECT * FROM news_posts WHERE id = $news_postsId";
                ...
                if ($resultCurrentData->num_rows > 0) {
                    ...
                    if (!empty($updateData)) {
                        ...
                        $sqlUpdate .= " WHERE id = $news_postsId";
                ...
                $sqlDelete = "DELETE FROM news_posts WHERE id = $news_postsId";
                ...
                $sqlAdd = "INSERT INTO news_posts (slug, title, short_content, content, views, updated_at) 
        ...
        ?>
        <?php
        // Truy vấn dữ liệu từ bảng news_posts
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                foreach ($columns as $column) {
                    ...
                    echo "<form method='post' action=''>
                            <input type='hidden' name='news_postsId' value='" . $row["id"] . "'>";
...
<style>
    .add-post-container {
        ...
        box-sizing: border-box;
        margin: 0 auto; /* Thêm dòng này để căn giữa theo chiều ngang */
    }
    ...
    button:hover {
        background-color: #333; /* Màu đen nhạt khi hover */
    }
    textarea {
        width: 100%;
        height: 100%; /* Thiết lập chiều cao là 100% để textarea có thể kéo rộng ra cả ngoài trang web */
        resize: both; /* Cho phép kéo rộng và kéo cao */
        padding: 8px;
        margin: 4px 0;
        border: 1px solid #ccc;
        border-radius: 4px;
