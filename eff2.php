...
<head>
    ...
    <title>Image Selection with Pixel Grid</title>
    <style>
        body {
            display: flex;
        }
...
<body>
    <h1>Image Selection with Pixel Grid</h1>
    ...
    <script>
            ...
            if (input.files && input.files[0]) {
                ...
                reader.onload = function (e) {
                    ...
                    image.onload = function () {
                        ...
                        // Vẽ lưới pixel
                        drawPixelGrid(ctx, image.width, image.height);
        ...
        function drawPixelGrid(ctx, imageWidth, imageHeight) {
            ctx.strokeStyle = '#ccc';
            ctx.lineWidth = 0.5;

            // Kích thước pixel sau khi phóng to
            var scaledPixelSizeX = imageCanvas.width / imageWidth;
            var scaledPixelSizeY = imageCanvas.height / imageHeight;

            // Vẽ lưới pixel
            for (var x = 0; x < imageCanvas.width; x += scaledPixelSizeX) {
                ctx.beginPath();
                ctx.moveTo(x, 0);
            ...
            for (var y = 0; y < imageCanvas.height; y += scaledPixelSizeY) {
                ctx.beginPath();
                ctx.moveTo(0, y);
