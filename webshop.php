...
if (isset($_POST['buyWebshop'])) {
    ...
    $result_player = $conn->query("SELECT name FROM players WHERE user_id = '$idnhanvat'");
    ...
    $result1 = $conn->query("SELECT * FROM players WHERE name = '$nhanvat'");
    ...
    if (!$players || $players['name'] == 0) {
        ...
        if ($coin < $giacoin) {
            ...
            if ($players['numberCellBag'] < count($bag)) {
                ...
                $players['bag'] = json_encode($bag);

                $itembuy = $players['bag'];

                $updateCoinQuery = "UPDATE users SET coin = " . ($coin - $giacoin) . " WHERE id = '" . $row_users["id"] . "'";
                //$updateCoinQuery = "UPDATE users SET coin = " . ($coin - $giacoin) . " WHERE name = '" . $nhanvat . "'";
                $up = $conn->query($updateCoinQuery);

                $updatebagQuery = "UPDATE players SET bag = '" . $itembuy . "' WHERE name = '" . $nhanvat . "'";
