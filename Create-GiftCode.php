...
<div class="card">
    <div class="card-body">
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            $code = $_POST["code"];
            $coin = empty($_POST["coin"]) ? 0 : $_POST["coin"];
            $gold = empty($_POST["gold"]) ? 0 : $_POST["gold"];
            $yen = empty($_POST["yen"]) ? 0 : $_POST["yen"];

            // Kiểm tra trùng lặp 'code' trước khi chèn
            $checkDuplicateSql = "SELECT COUNT(*) FROM gift_codes WHERE code = ?";
            $checkDuplicateStmt = $conn->prepare($checkDuplicateSql);
            $checkDuplicateStmt->bind_param("s", $code);
            ...
            if ($count > 0) {
                ...
                document.addEventListener("DOMContentLoaded", function() {
                    var modal = document.getElementById("activeThatBai6");
                    var modalContent = document.getElementById("activeThatBaiContent6");
                    modalContent.innerHTML = "<span style=\"font-size: 18px; font-weight: bold; color: #ff0000;\">Mã quà tặng đã tồn tại!</span>";
                ...
                if (!empty($_POST["id"])) {
                    for ($i = 0; $i < count($_POST["id"]); $i++) {
                        ...
                        $expire = intval($_POST["expire"][$i]);
                        $sys = intval($_POST["sys"][$i]);
                        $quantity = intval($_POST["quantity"][$i]);
                        $options = $_POST["options"][$i];
                        $isLock = $_POST["isLock"][$i];
                        ...
                        $newItem = array(
                            ...
                            "isLock" => ($isLock === 'true')  // Chuyển đổi chuỗi 'true' thành true, 'false' thành false
                        );
                ...
                $insertSql = "INSERT INTO gift_codes (code, coin, gold, yen, items, server_id, type, status) VALUES (?, ?, ?, ?, ?, 1, 1, 0)";
                $insertStmt = $conn->prepare($insertSql);
                $insertStmt->bind_param("sssss", $code, $coin, $gold, $yen, $itemsJson);

                if ($insertStmt->execute()) {
                    echo '<script>
                    document.addEventListener("DOMContentLoaded", function() {
                        var modal = document.getElementById("activeThanhCong6");
                        var modalContent = document.getElementById("activeThanhCongContent6");
                        modalContent.innerHTML = "<span style=\"font-size: 18px; font-weight: bold; color: #0096ff;\">Thêm GiftCode thành công</span>";
                        modal.style.display = "block";
                    });
                    </script>';
                    echo "<script>openActiveThanhCong6();</script>";
                } else {
                    echo "Lỗi: " . $insertStmt->error;
                    echo "<script>openActiveThatBai6();</script>";
                }
        ...
        <div class="card1">
            <div class="card-body1">
                <h2 class="text-center" style="color: #0096ff; font-weight: bold;">Thêm Gift Code</h2>
<!--start đổi ngày-->      
...
<br>
<!--ending đổi ngày-->
                <form method="post" action="" onsubmit="return validateForm()" id="itemTable">
                    ...
                    <div class="form-group row">
                        <label class="col-sm-2 col-form-label">Xu:</label>
                        <div class="col-sm-10">
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-10">
                            <input type="text" name="yen" class="form-control">
                        </div>
                    </div>

                    <div class="form-group row">
                        <label class="col-sm-2 col-form-label">Lượng:</label>
                        <div class="col-sm-10">
                            <input type="text" name="gold" class="form-control">
                        </div>
                    </div>

                    <!-- Trường nhập liệu cho items -->
                    <div id="itemsContainer" class="form-group"></div>
                    <button type="button" onclick="addItem()" class="btn btn-primary">Thêm Item</button><br>

                    <!-- Ô nhập liệu giá trị tổng cho items -->
                    <div class="form-group" style="display: none;">
                        <label>Total:</label>
                        <input type="text" id="totalItems" readonly class="form-control">
                    </div>

                    <div class="form-group row">
                        <div class="col-sm-12 text-center">
                            <input type="submit" value="Thêm Gift Code" class="btn btn-success">
                        </div>
                    </div>
                </form>
            </div>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body text-center">
                <a href="/"><img class="logo" alt="Logo" src="/images/logoweb.gif" style="max-width: 250px;"></a>
