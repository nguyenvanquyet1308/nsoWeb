...
if(isset($_GET['act']))
{
	...
	if($act=='card')
	{
		...
		$read = _fetch(_select('*','tongnap',"code='$manap' && name='$tenthe' && seri='$seril'"));
			$txt = _insert('tongnap',"type,name,code,seri,number,user,time_in,status","'card','$tenthe','$manap','$seril','$menhgia','$_username','".time()."','wait'");
			$add = _query($txt);
			if($add)
			{
