...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "SELECT players.user_id, players.name, users.status, users.kh, users.coin, users.luong, users.isVIP, users.tongnap, users.ip_address, users.password, users.phone
            FROM players 
...
<style>
    .form-signin {
        max-width: 300px; /* Điều chỉnh chiều rộng tối đa của ô tìm kiếm */
        margin: 0 auto; /* Canh giữa ô tìm kiếm */
    }

    .form-control {
        max-width: 100%; /* Chiều rộng tối đa của ô tìm kiếm */
    }
</style>
<style>
    .blink {
...
<?php include('error.php'); ?>
        ...
        <div id="result-container">
            ...
            if (isset($result)) {
                if ($result->num_rows > 0) {
                    ...
                    while ($row = $result->fetch_assoc()) {
                        ...
                        echo "<tbody>";
                        echo "<tr class='fw-semibold'><td>ID</td><td>" . $row["user_id"] . "</td></tr>";
                        ...
                        if ($row["isVIP"] > 0) {
                        echo "<tr class='fw-semibold'><td class='blink' style='color: rgb(255, 102, 0);'>VIP</td><td class='blink' style='color: rgb(255, 102, 0);'>" . $row["isVIP"] . "</td></tr>";
                        } else {
                        echo "<tr class='fw-semibold'><td>VIP</td><td>" . $row["isVIP"] . "</td></tr>";
                        }                        
           ...
           //                echo "<a class='cursor-pointer text-primary show-all-ip' style='display:$showAllLinkStyle;'><span> </span>Hiển thị Tất cả</a>";
            ...
            function getActivatedDescription($kh) {
                $color = ($kh == 1) ? "#0096ff" : "red";
                return "<span style='color: $color;'>".($kh == 1 ? "Đã kích hoạt" : "Chưa kích hoạt")."</span>";
            }
