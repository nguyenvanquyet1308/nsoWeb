...
try {
    ...
    $row_users = $conn->query("SELECT * FROM users WHERE username = '$username' LIMIT 1");
    ...
    if ($row_users != false && $row_users->num_rows > 0) {
        ...
        $update_query = "UPDATE users SET kh = 1, coin = $new_coin_balance WHERE username = '$username'";
