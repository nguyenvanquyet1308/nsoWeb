...
try {
    $checkUserResult = mysqli_query($conn, "SELECT username FROM users WHERE username = '".$username."' AND phone = '".$phone."' LIMIT 1");
    ...
    if($checkUserResult == false || mysqli_num_rows($checkUserResult) < 1){
        ...
        $sql = mysqli_query($conn, "UPDATE `users` SET `password` = '".$code."', `updated_at` = '".date("Y-m-d H:i:s")."' WHERE `username` = '".$username."' AND `phone` = '".$phone."'");
