...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (checkAdmin($conn, $_SESSION['username'])) {
    if (isset($_POST["edit"])) {
        $gioithieuId = $_POST["gioithieuId"];
        $newContent = $_POST["newContent"];

        $sqlUpdate = "UPDATE gioithieu SET content = '$newContent' WHERE id = $gioithieuId";

        if ($conn->query($sqlUpdate) === TRUE) {
            // Lấy địa chỉ IP của người dùng
            $ipAddress = $_SERVER['REMOTE_ADDR'];

            // Hiển thị Giới thiệu bằng mã JavaScript
            echo "<script>
                if ('$ipAddress') {
                    var notification = document.createElement('div');
                    notification.innerHTML = 'Sửa Giới thiệu thành công';
...
<style>
    ...
    .notification {
        ...
        /* CSS để căn giữa phần sửa Giới thiệu */
        .edit-container {
        width: 50%;
        margin: auto;
        text-align: center;
...
<center>
    <h2 style="color: black;">Sửa Giới thiệu</h2>
</center>

<?php
// Truy vấn dữ liệu từ bảng gioithieu
$sqlSelect = "SELECT * FROM gioithieu";
    ...
    while ($row = $result->fetch_assoc()) {
        echo "<div class='edit-container'>
                <form method='post' action=''>
                    <input type='hidden' name='gioithieuId' value='" . $row["id"] . "'>
