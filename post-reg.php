...
try {
    ...
    if ($result != false && $result->num_rows > 0) {
        ...
        if ($result != false && $result->num_rows > 0) {
            // Email đã tồn tại
            echo '{"code": "03", "text": "Email đã tồn tại trên hệ thống."}';
        } else {
            // Thực hiện thêm mới người dùng với created_at và đặt giá trị "vip" là 1
            $stmt = $conn->prepare("INSERT INTO users (username, password, phone, created_at, status, activated, kh) VALUES (?, ?, ?, ?, 1, 1, 0)");
