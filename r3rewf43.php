...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (checkAdmin($conn, $_SESSION['username'])) {
    $NameNV = $_POST["users_name"];
    $itemId = $_POST["id"];

    $checkOnlineQuery = "SELECT u.id, u.online FROM users u
    ...
    $sql = "SELECT * FROM players WHERE name = '$NameNV'";
    $result = $conn->query($sql);

    if ($result->num_rows > 0) {
        $row = $result->fetch_assoc();
        $bagData = json_decode($row["bag"], true);
        ...
        $updateSql = "UPDATE players SET bag = '$updatedbag' WHERE name = '$NameNV'";
...
<div class="card">
    <div class="card-body">
        ...
        <form method="post" action="">
            Tên nhân vật <input type="text" name="users_name" required><br>
